...
from .utils import altitude_to_airmass, RA_to_HA, HA_to_RA
...
from .utils import skycoord_to_altaz
...
class QueueManager(object):
    ...
    def determine_allowed_requests(self, time, obs_log,
        ...
        self.logger.debug(
            f"Sum of change in allowed exposures by subprogram: {delta_subprogram_exposures_tonight.reset_index().groupby('program_id').agg(np.sum)}")
        ...
        for op in self.observing_programs:
            ...
            if op.subprogram_name == 'i_band':
                ...
                if delta_i_nexp > 0:
                    ...
                    self.logger.info(
                        f'Implied change in i-band exposures is negative, skipping supplementation: {delta_i_nexp}')
...
class GurobiQueueManager(QueueManager):
    ...
    def _assign_slots(self, current_state, time_limit=30 * u.second,
        ...
        nscheduled_requests_bysubprogram = \
            dft.loc[dft['scheduled'], ['program_id', 'subprogram_name']].groupby(['program_id', 'subprogram_name']).agg(
                len)
    ...
    def _move_requests_to_missed_obs(self, queue_slot):
        ...
        if len(self.queue):
            ...
            self.logger.info(
                f"Saving {len(missed_obs)} requests (filter {filter_id}) to the missed_obs queue: {missed_obs.loc[:, ['subprogram_name', 'field_id']]}")
