...
def msip_nss_selection_phaseii(time, obs_log, other_program_fields, fields,
    ...
    nss_footprint_field_ids = candidate_nss_fields.sort_values(by='abs_HA_midnight', ascending=True).iloc[
                              :(msip_cadence * n_fields_to_observe - 1)].index.tolist()
    # save these if we need filler
    candidate_field_ids_outside_footprint = candidate_nss_fields.sort_values(by='abs_HA_midnight', ascending=True).iloc[
                                            (msip_cadence * n_fields_to_observe - 1):].index.tolist()
    ...
    if len(nss_field_ids_due) * msip_nobs_per_night > nss_requests_allowed:
        ...
        if not silent:
            logger.info(
                f'MSIP NSS: {nss_requests_allowed / msip_nobs_per_night} fields needed, {len(nss_field_ids_due)} available--removing fields.')
        ...
        nss_field_ids_to_observe = available_nss_fields.sort_values(by='expMJD', ascending=True).iloc[
                                   :n_fields_to_observe].index.tolist()
        ...
        if not silent:
            logger.info(
                f'MSIP NSS: {nss_requests_allowed / msip_nobs_per_night} fields needed, {len(nss_field_ids_due)} available--adding fields.')
...
def phase_II_selection(time, obs_log, other_program_fields, fields,
    ...
    candidate_field_ids_byprio = [candidate_field_ids_prio1, candidate_field_ids_prio2, candidate_field_ids_prio3,
                                  candidate_field_ids_prio4]
    ...
    if subprogram == 'Partnership':
        ...
        offset = 15
        exclude_fields = phase_II_selection(time, obs_log, other_program_fields, fields, subprogram='Partnership',
                                            silent=False)
    ...
    for i, prio_field_ids in enumerate(candidate_field_ids_byprio):
        ...
        if len(visible_prio_field_ids) <= offset:
            ...
            if i == 0:  # prio 1
                ...
                if not silent:
                    logger.info(
                        f'{label}: requesting {len(visible_prio_field_ids)} priority {i + 1} fields: {visible_prio_field_ids}')
        ...
        if (len(visible_prio_field_ids) + offset) <= n_fields_remaining:
            ...
            if not silent:
                logger.info(
                    f'{label}: requesting {len(visible_prio_field_ids[offset:])} priority {i + 1} fields: {visible_prio_field_ids}')
        else:
            ...
            if not silent:
                logger.info(
                    f'{label}: requesting {len(footprint_field_ids)} priority {i + 1} fields of {len(candidate_fields)} total: {footprint_field_ids}')
...
def srg_selection(time, obs_log, other_program_fields, fields):
    ...
    nss_selection_function = globals()[
        other_program_fields[(PROGRAM_NAME_TO_ID['MSIP'], 'all_sky')]['field_selection_function']]
            ...
            if key[1] != 'all_sky':
                if oops['field_ids'] is not None:
                    ...
                    try:
                        ...
                        logger.exception(e)
                        logger.warning(
                            f'During MSIP NSS field selection, error in generating nightly field list for {key}')
