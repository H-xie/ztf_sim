...
def night_optimize(df_metric, df, requests_allowed, time_limit=30 * u.second,
    ...
    if not space_obs:
        ...
        m.setObjective(
            ...
            - ydfds.sum() * (FILTER_CHANGE_TIME / (EXPOSURE_TIME +
                                                   READOUT_TIME) * 2.5).value
            + np.sum(yrdtf[r, dt, f] * slot_scale(dt) for r in ZUDS_request_sets for dt in dtdict.keys() if
                     dt >= MIN_SLOT_SEPARATION for f in filter_ids_to_limit)
            - np.sum(
                [heaviside((requests_allowed[p] - np.sum(
                    dft.loc[(dft['program_id'] == p[0]) &
                            (dft['subprogram_name'] == p[1]), 'Yrtf'].values
    ...
    while df_schedule.groupby(['slot', 'request_id']).agg(len).max()[0] > 1:
        ...
        print("> Slot optimization did not satisfy all constraints. Continuing Optimization (Iteration {})".format(
            n_iterations))
        m.update()
        m.optimize()
